...
def setup_exception_handlers(app: Any) -> None:
    ...
    Converts our custom exception hierarchy (and any uncaught error) into
    ...
    @app.exception_handler(AIBoardroomException)
    async def _handle_ai_boardroom_exception(request: Request, exc: "AIBoardroomException") -> JSONResponse:
        if exc.status_code >= 500:
            logger.error("Unhandled application error", path=request.url.path, error=exc.message)
        return JSONResponse(
            status_code=exc.status_code,
            content={"detail": exc.message, "errors": exc.detail},
        )
    ...
    async def _handle_unexpected_exception(request: Request, exc: Exception) -> JSONResponse:
        logger.error("Unexpected server error", path=request.url.path, error=str(exc))
